...
from inspect_flow._config.portable import SpecNotPortableError, validate_portable_spec
...
def _venv_spawn(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        with RunAction("env", info="venv") as action:
            ...
            except SpecNotPortableError as error:
                raise SpecNotPortableError(
                    error.violations,
                    hint="Alternatively, run using the 'inproc' execution type, which supports live objects.",
                ) from None
