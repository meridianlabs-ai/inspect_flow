...
from inspect_ai.analysis import prepare
...
def create_type(defs: Schema, title: str, base_type: Schema, type: GenType) -> None:
    ...
    if new_title.startswith("F"):
        new_title = "Flow" + new_title[1:]
...
def generate_dict_code() -> list[str]:
    ...
    with tempfile.NamedTemporaryFile(mode="w+", suffix=".py") as tmp_file:
        ...
        lines = modify_generated_code(dict_lines, "dict")
        ...
        lines.extend(modify_generated_code(pydantic_lines, "pydantic"))
...
def modify_generated_code(
    lines: list[str], mode: Literal["dict", "pydantic"]
) -> list[str]:
...
def main():
    ...
    write_generated_code("dicts2.py", lines)
