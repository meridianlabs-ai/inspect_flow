...
| `--handle-file` | file | Write a JSON launch handle (with the run’s `log_dir` and `pid`) to this file once the log directory is resolved. Lets a monitor discover a backgrounded run. A relative path is resolved relative to the config file’s directory, not the current working directory. | None |
