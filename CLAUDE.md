...
- **Dependencies**: Do not pin dependency versions in `pyproject.toml`. inspect_flow is a library that must install alongside many different environments, so version pins (or upper bounds) needlessly restrict downstream usage and cause resolver conflicts. Leave `[project] dependencies` unconstrained (raise a `>=` floor only when flow genuinely requires it) and let `uv.lock` provide reproducibility. If a transitive dependency must be held back temporarily, prefer a `[tool.uv] constraint-dependencies` entry over a published dependency bound.
