...
### [**tag**](reference/inspect_flow.api.qmd#tag)
...
### [**metadata**](reference/inspect_flow.api.qmd#metadata)
...
### [**copy**](reference/inspect_flow.api.qmd#copy)
...
The `scan` step runs after the eval set completes. To instead run scanners inline against each sample *during* the run, set `scanner` on `FlowOptions` to a path to a scanner config file (or a `ScannerConfig`) — this maps to the `scanner` parameter of Inspect's `eval_set()`. The config file path is resolved relative to the config file (CLI) or `base_dir` (API), but any `file:` references *inside* the scanner config are resolved by Inspect Scout relative to the current working directory, so prefer absolute `file:` paths when running from a different directory.
...
Steps can return a plain `list[EvalLog]` or a [`StepResult`](reference/inspect_flow.api.qmd#StepResult) for finer control:
...
The same `@log_filter` functions work across Flow — with [`flow list log`](reference/flow_list.qmd), [`flow run --store-filter`](store.qmd#filtering), and store commands. Like steps, filters defined or imported in `_flow.py` files are automatically discovered.

## Checking Completeness {#checking-completeness}

The [`flow check`](reference/flow_check.qmd) command checks the completeness of a spec against existing logs in a directory. Its primary use is checking against a different log directory than the one in the spec — for example, checking how complete a production directory is:
...
See the [API Reference](reference/inspect_flow.api.qmd) for complete documentation.
