...
The `scan` step runs after the eval set completes. To instead run scanners inline against each sample *during* the run, set `scanner` on `FlowOptions` to a path to a scanner config file (or a `ScannerConfig`) — this maps to the `scanner` parameter of Inspect's `eval_set()`.
