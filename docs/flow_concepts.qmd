...
-   [**`FlowScorer`**](/reference/inspect_flow.html#flowscorer) — Configuration for a Scorer. Map to Inspect AI [Scorer](https://inspect.aisi.org.uk/scorer.html).
...
In [Basic Example](index.qmd#basic-example), we showed a simple `FlowJob`. Let's break down what's happening:
...
| `defaults` | Default values applied across tasks, models, solvers, and agents (see [Defaults and Overrides](defaults_overrides.qmd) and `FlowDefaults` reference) | `None` |
...
Python config files are evaluated as normal Python code. The last expression in the file should either be a `FlowJob` or a function that returns a `FlowJob`. This means you can:
...
`FlowTask` accepts parameters that map to Inspect AI [Task](https://inspect.aisi.org.uk/tasks.html) fields. The examples below show commonly used fields; see the [`FlowTask`](/reference/inspect_flow.html#flowtask) reference documentationfor the complete list of available parameters.
