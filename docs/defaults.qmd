...
::: callout-note
When using `run()` from the Python API to run the `FlowJob` directly instead of the command line, the `base_dir` argument is used as the starting point for searching upward through the directory tree, rather than the config file's location.
:::
...
**Runtime-only flags**: `INSPECT_FLOW_LOG_LEVEL`, `INSPECT_FLOW_ARG`, `INSPECT_FLOW_NO_VENV`, `INSPECT_FLOW_DRY_RUN`, and `INSPECT_FLOW_SET` (and their corresponding CLI flags `--log-level`, `--arg`, `--no-venv`, `--dry-run`, `--set`) are runtime settings for the `flow run` command and cannot be set in `FlowJob`.
