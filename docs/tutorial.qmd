...
-   **`FlowConfig`** — The top-level workflow definition that translates to an Inspect AI [Eval Set](https://inspect.aisi.org.uk/eval-sets.html). Contains a list of tasks and workflow settings.
...
`FlowConfig` is the primary interface for defining Flow workflows. All Flow operations—including [parameter sweeps and matrix expansions](#parameter-sweeping)—ultimately produce a list of tasks that `FlowConfig` executes.
...
In [Basic Examples](index.qmd#basic-examples), we showed a simple `FlowConfig`. Let's break down what's happening:
...
Python config files are evaluated as normal Python code. The last expression in the file is used as the `FlowConfig`. This means you can:
...
4.  **Generation config** — Maps to Inspect AI `Task.config` (`GenerateConfig`). Model generation parameters like `temperature`, `max_tokens`, `top_p`, `reasoning_effort`, etc. These settings override config on `FlowConfig.config` but are overridden by settings on `FlowModel.config`.
...
Inspect Flow automatically creates isolated virtual environments for each workflow run, ensuring reproducibility and avoiding dependency conflicts with your system Python environment.
...
The `dependencies` field in `FlowConfig` accepts multiple types of package specifiers:
...
Install directly from Git repositories. Use `@commit_hash` to pin to specific versions for reproducibility.
...
Specify the Python version for your workflow's virtual environment:
...
``` python
from inspect_flow.types import FlowConfig, FlowDefaults, FlowGenerateConfig, FlowModel
...
1.  Default model generation options. Will be overriden by settings on the FlowModel and FlowTask.
...
The final configuration follows this priority (highest last):

1.  Config file defaults
2.  Task/model/solver specific settings
3.  Environment variables
4.  CLI `--set` flags

CLI flags always win, making them perfect for quick experiments without modifying config files.
:::
...
``` python
from inspect_flow import tasks_matrix, models_matrix
from inspect_flow.types import FlowConfig, FlowGenerateConfig
...
``` python
from inspect_flow import tasks_matrix, models_matrix, configs_matrix
from inspect_flow.types import FlowConfig
...
``` python
from inspect_flow import tasks_matrix, solvers_matrix
...
``` python
from inspect_flow import tasks_with, tasks_matrix, configs_matrix
from inspect_flow.types import FlowConfig, FlowGenerateConfig
...
``` python
from inspect_flow.types import FlowConfig, FlowTask, FlowOptions
...
The `flow_metadata` field is available on `FlowTask`, `FlowModel`, `FlowSolver`, and `FlowAgent`. This metadata is **not passed to Inspect AI**—it exists only in the Flow configuration and is useful for configuration-time logic and organization.
...
``` python
from inspect_flow import tasks_matrix
from inspect_flow.types import FlowConfig, FlowModel
