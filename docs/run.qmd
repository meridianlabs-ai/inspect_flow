...
Writes the job configuration (as yaml) without running any tasks. Useful for validating large parameter sweeps.
...
Inspect View can automatically detect Flow config files in the log directory and render them in the UI, making it easier to review the job configuration for the evaluations.
...
The latest Flow config file is rendered in YAML format.
