...
- **`run()`** - Execute a Flow job with full environment setup (equivalent to `flow run`)
- **`load_job()`** - Load a Flow configuration from a Python file into a `FlowJob` object
...
[Inspect View](https://inspect.aisi.org.uk/log-viewer.html) can automatically detect Flow config files in the log directory and render them in the UI, making it easier to review the job configuration for the evaluations.
